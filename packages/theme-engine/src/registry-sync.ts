...
/**
 * Upserts the built-in section registry into section_definitions (store_id null), with each
 * definition's placements, module and policy tags. Idempotent. The registry was validated when
 * the definitions module loaded, so an untagged definition never reaches the table.
 */
  ...
  await withPlatformTx(db, async (tx) => {
    for (const def of SECTION_DEFINITIONS) {
      ...
      const values = {
        ...
        propTags: Object.fromEntries(Object.entries(def.propTags).map(([k, v]) => [k, [...v]])),
      };
